...
const OrderBoard = () => {
  ...
  const fetchOrders = () => {
    fetch('https://aiac3.onrender.com/api/orders')
      .then(res => res.json())
      .then(data => {
  ...
  const markPrepared = async (id) => {
    await fetch(`https://aiac3.onrender.com/api/orders/${id}/status`, {
      method: 'PATCH',
  ...
  const handleVerify = async (order) => {
    ...
    if (enteredToken && enteredToken.toUpperCase() === order.shortToken) {
      await fetch(`https://aiac3.onrender.com/api/orders/${order.id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
