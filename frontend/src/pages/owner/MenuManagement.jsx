...
const MenuManagement = () => {
  ...
  const fetchMenu = () => {
    fetch('https://aiac3.onrender.com/api/menu')
      .then(res => res.json())
  ...
  const handleAddItem = async (e) => {
    e.preventDefault();
    await fetch('https://aiac3.onrender.com/api/menu', {
      method: 'POST',
  ...
  const handleDelete = async (id) => {
    await fetch(`https://aiac3.onrender.com/api/menu/${id}`, { method: 'DELETE' });
    fetchMenu();
  };

  const toggleAvailability = async (item) => {
    await fetch(`https://aiac3.onrender.com/api/menu/${item.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
